...
it('Gets and sets storage values from default', async () => {
    const { openStore, getItem, setItem, removeItem, clear,
...
it('Gets and sets storage values from one store & two tables', async () => {
    const { openStore, getItem, setItem, removeItem, setTable,
            getAllKeysValues, isAvailable } = useStorageSQLite(); 
...
it('Gets and sets storage values Json Object', async () => {
    const { isJsonValid, importFromJson, exportToJson, isAvailable } = useStorageSQLite(); 
